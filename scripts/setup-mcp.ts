...
/**
 * MCP Server Configuration Helper (Gemini Version)
 */
...
console.log("╔════════════════════════════════════════════════════════════════╗");
console.log("║   Fuzion AI Knowledge Base MCP Server Configuration Helper    ║");
console.log("╚════════════════════════════════════════════════════════════════╝\n");
...
const envPath = path.join(projectPath, ".env");
let postgresUrl = process.env.POSTGRES_URL || "";
let geminiKey = process.env.GOOGLE_API_KEY || "";
...
if (envFile) {
  ...
  const postgresMatch = envContent.match(/POSTGRES_URL=(.+)/);
  const geminiMatch = envContent.match(/GOOGLE_API_KEY=(.+)/);

  if (postgresMatch) postgresUrl = postgresMatch[1].trim().replace(/["']/g, "");
  if (geminiMatch) geminiKey = geminiMatch[1].trim().replace(/["']/g, "");
} else {
  console.log("⚠️  No .env or .env.local file found\n");
}
  ...
  mcpServers: {
    "fuzion-ai-knowledge": {
      ...
      env: {
        POSTGRES_URL:
          postgresUrl || "postgresql://user:pass@localhost:5432/fuzionai",
        GOOGLE_API_KEY: geminiKey || "AIzaSyAwm7Q-m6_sjnCSDcsK9GfOXKWsyAwTu7E",
      },
    },
  },
};
...
console.log("═══════════════════════════════════════════════════════════════════");
console.log("⚠️ Scoped Access Configuration:");
console.log("═══════════════════════════════════════════════════════════════════\n");
...
// Warnings
const warnings: string[] = [];
if (!postgresUrl) warnings.push("⚠️  POSTGRES_URL not found");
if (!geminiKey) warnings.push("⚠️  GOOGLE_API_KEY not found");
